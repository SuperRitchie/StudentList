...
void dslist::sortinsert(DomesticStudent& stu) {

    DomesticStudent* student = NULL;
    try{
      student = new DomesticStudent(stu);
    }
    catch(bad_alloc &ba)
    {
      ...
      if(head->get_researchscore() < student->get_researchscore() || head->get_researchscore() == student->get_researchscore())
      {
        student->next = head;
        head = student;
        return;
      ...
      while(temp != NULL)
      {
        if (temp->get_researchscore() > student->get_researchscore() )
        {
          ...
          else if (student->get_researchscore() > temp->next->get_researchscore() || student->get_researchscore() == temp->next->get_researchscore())
          // Insertion position found, place in the middle of temp and temp->next
          {
            DomesticStudent* temp2 = temp->next;
            temp->next = student;
            student->next = temp2;
            //delete temp2;
            break; 
          ...
          {
            cout << endl << temp -> get_firstname() << endl;
            temp = temp->next;
          }
        }
        else {
          if (student->get_researchscore() == temp->get_researchscore() || student->get_researchscore() > temp->get_researchscore())
          {
            if (temp->next != NULL)
            {
              DomesticStudent* temp2 = temp->next;
